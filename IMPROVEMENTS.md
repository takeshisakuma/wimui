...
| T54 | `check:api` が prop の型の縮小を見ない | **未着手**（2026-08-02 起票・**P2**） | T38（0.13.0）で `Card` の `padding` / `radius` の union を狭めた。これは**利用者のコードがコンパイルしなくなる**変更だが、`check:api` v2 は「exports マップ + エントリの symbol」を見るので**スナップショットは無風のまま通った**。つまり公開 API の破壊のうち、**シンボルの増減を伴わない種類（prop の型・必須/任意・戻り値）は誰も見ていない**。T38 は人間が破壊と分かって出したから問題にならなかっただけで、意図せず狭めたときに気付ける仕組みは無い | 案①: `api-snapshot.json` に **prop シグネチャの層**を足す（docgen の tsType を正規化して保存し、差分が出たら changeset で破壊を宣言したか確認を促す）。全 223 コンポーネント分を凍結するとノイズが多いので、**公開バレルから出るコンポーネントの props だけ**に絞る ／ 案②: `dist/**/*.d.ts` に対して API Extractor 相当の差分を取る（正確だがビルド必須で遅い） ／ **受け入れ条件**: T38 の変更（`Card.radius` から `5xl` が消える）を**過去のコミットで再現して鳴らせること**。あわせて**鳴ってはいけない経路**＝prop の追加（加算・非破壊）では鳴らないこと |
