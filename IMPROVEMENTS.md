...
**次はここから。順番は決定済み**: ①**T61**（T59 の同型。`container-type` 側。**19 件**。**T60-② を内包**）→ ②**T55** → ③**T57** → ④T63 → ⑤T64。**T62 は 2026-08-03 に完了**（3 件とも修正・7 経路を実測）。T32 の 4 枚目（モバイル前提のレイアウト）はそのあと。**この順にした理由は 08-03 朝と変わらない**（4 枚目でフォーム部品を横並びに置くと踏む形を先に潰す）が、**T59 を潰したら同じ穴の残り半分が出てきた**ので、そちらが先。**T60-② は T61 では直らないことを実測済み**（`ChatUI` の根は 390px で縮んでいない。削っているのは `.message` の `max-width: 80%` とアバター）。②は `ChatUI` 固有の狭幅設計として別途扱うこと。<br>**T59 / T60 は完了**（#229。VRT のベースラインも 134 枚を update 済み）。**T62 も完了**（#230）。
