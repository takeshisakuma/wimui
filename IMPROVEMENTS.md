...
最終更新: 2026-07-26（**T27〜T31** を起票。T27 Playground 再設計 + AI-slop ガード拡張(#108) / T28 コンポジション監査 + T30 prop 検出ガード(#109) / T29 docs の px 負債(#111) は完了。T31（docgen の CACHE_VERSION 上げ忘れ検知）が未着手。※起票時に T14〜T17 と番号が衝突していたため T27 以降へ採番し直し）  
...
2. **T31: docgen の CACHE_VERSION 上げ忘れを機械検知**（2026-07-26 起票・未着手）  
   `scripts/docgen-plugin.js` のパース／描画ロジックを変えたのに `CACHE_VERSION` を上げないと、**ローカルの暖かいキャッシュだけが古い出力を返し続ける**。CI はクリーンチェックアウトなので気づけない。詳細は下表 T31。
