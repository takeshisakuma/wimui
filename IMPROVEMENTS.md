...
**ベースラインの枚数は「変更が何を動かしたか」の指標にならない**（#189 の update で判明）。コミットバックされた **99 枚のうち、この変更に起因するのは 10 枚だけ**だった（除外解除の 4 枚＋`tabnavigation--default` / `tabs--scrolling` / `togglegroup--sizes` / `--full-width`）。残り 89 枚を 1 枚ずつ測ると **81 枚は `maxDiffPixels: 50` 未満**＝compare では絶対に落ちない churn（多くは 1px・YIQ 距離 1）。`--update-snapshots=all` は全ファイルを撮り直すので、**閾値未満の揺れまでコミットに載る**。当初「#187 のベースライン更新が走っていないせいでは」と考えたが**外れ**で、#187 は 195 枚を更新済みだった。**50 を超えた 8 枚は要注意**＝`aspectratio--default`(844) / `audio--default`(589) / `aspectratio--square`(295) / `avatargroup--total-count`(252) / `hovercard--default` dark(148)・light(132) / `video--default`(120) / `otpinput--danger`(95)。**全部が媒体・非同期系**で、`avatargroup` と `video` は兄弟ストーリーが既に除外リストにある。**次の compare を赤にしうる**ので、赤が出たらまずここを疑うこと（T43 の後継候補）。
