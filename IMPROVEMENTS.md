...
**2026-08-03 夜時点の順番（決定済み）**: ①T61（`container-type` 側の同型 20 件。**T60-② を内包**）→ ②T55（VRT が外部画像に依存）→ ③T57（`AppShell` の Composition API）→ ④T63 → ⑤T64 → ⑥T32 の 4 枚目。理由は冒頭の「次はここから」を参照。**T59 / T60 は同日に完了**（T59 は 0 件でハードゲート化、T60 は ② を T61 へ移管して閉じた）。**両方とも VRT のベースライン更新だけ未実施。**
