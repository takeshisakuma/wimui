...
最終更新: 2026-07-29（**0.7.0 を公開**＝T32 の 2 枚目で出た穴 14 件のうち 11 件を出荷した「既定が変わる回」。あわせて **T45**（light パレットに面の分離が無く、コンポーネントが影に依存している）を起票。次の一手は下の「**ハーネス修正の 1 回目は失敗した**（T44 の計測結果の節・明日はここから）」）。**同日続き: 回避策 4 つをすべて外した**（結果は同節「回避策を外した結果」。⑦ の残りは `SegmentedControl` を「省略せず横スクロール」へ倒して解消。ライブラリ=#165・画面=#166 として出荷済み。**T32 の 2 枚目はこれで完了**、次は 3 枚目（AI アシスタント画面））  
...
**T38 は 0.8.0。そして 0.8.0 を「型が真実を語る回」と定義する。** 0.6.0 → 0.7.0 → 0.8.0 と 2 回延期しているので、3 回目を防ぐために定義と代償措置をセットにする。

- **延期の理由は今回も同じ**: 0.7.0 は ④⑤ で「見た目の既定値が変わる回」になる。union の縮小は**コンパイルが止まる**性質なので、混ぜると利用者が原因を切り分けられない（0.6.0 から外したときと同じ理屈）
- **危険なのは「カテゴリで延期する」に自然な停止点が無いこと**。どのリリースにも見た目の変更は多少入るので、同じ理屈で無限に延期できる。だから 0.8.0 を**型の回**と名前で固定し、同種のもの（`api-snapshot.json` が動く縮小）はそこへ集める
...
##### ハーネス修正の 1 回目は失敗した（2026-07-29 夜。**明日はここから**）

`fonts.load` + インジケータ待ちを入れて計測ランを 2 本流した結果、**この修正は非決定性を減らしていない**。
...
**見立て（未検証）**: `document.fonts.load()` は、その時点で `@font-face` が**未登録なら何も待たずに解決する**。Google Fonts のスタイルシートは非同期に読まれるので、登録前に呼ぶと素通りになる。前版（`document.fonts.check` のポーリング）が遅かったのは「登録されるまで待ち続けていた」ためで、**遅さと有効性が同じ理由から来ていた**可能性が高い。run4 でだけ落ちた顔ぶれ（`Breadcrumb` / `Grid` / `Span` / `Table` など互いに無関係な多数）は、**テキスト全体がずれる＝フォント差し替え**の兆候と整合する。

**次に試す形**: `page.goto` を `waitUntil: "load"` にしてスタイルシート到着を待ってから `document.fonts.load()` を呼ぶ（必要なら `check()` の短周期ポーリングを上限つきで併用）。**評価は必ず 2 ラン**で行う。

**教訓（今日 2 回目の同型ミス）**: **1 ラン見て「効いた」と言わない。** 非決定性の評価には最低 2 ラン要る。同日、`SegmentedControl` の既定経路でも「修正後しか測っていない」で同じ誤りをしている（2 枚目の記録参照）。

**PR の状態**: **#171 は draft のまま。マージしない**（効果が実測で否定されたため）。CI は 13 チェック緑で、所要時間の悪化も無い（VRT compare 6m35s〜6m49s＝通常どおり）ことは確認済みなので、フォント待ちの形だけ直せば再利用できる。
