...
**次はここから。順番は決定済み**: ①**T61**（T59 の同型。`container-type` 側。**19 件**。**T60-② を内包**）→ ②**T55** → ③**T57** → ④T63 → ⑤T64。**T62 は 2026-08-03 に完了**（3 件とも修正・7 経路を実測）。T32 の 4 枚目（モバイル前提のレイアウト）はそのあと。**この順にした理由は 08-03 朝と変わらない**（4 枚目でフォーム部品を横並びに置くと踏む形を先に潰す）が、**T59 を潰したら同じ穴の残り半分が出てきた**ので、そちらが先。**T61 は `ChatUI` から測ること** — T60-② の真因がこれで説明できるなら一緒に直る。<br>**T59 / T60 は完了**（#229。VRT のベースラインも 134 枚を update 済み）。**T62 も完了**（#230）。
...
**2026-08-03 夜時点の順番（決定済み）**: ①T61（`container-type` 側の同型 19 件。**T60-② を内包**）→ ②T55（VRT が外部画像に依存）→ ③T57（`AppShell` の Composition API）→ ④T63 → ⑤T64 → ⑥T32 の 4 枚目。理由は冒頭の「次はここから」を参照。**T59 / T60 は同日に完了**（T59 は 0 件でハードゲート化、T60 は ② を T61 へ移管して閉じた）。**両方とも VRT のベースライン更新だけ未実施。**
