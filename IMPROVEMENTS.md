...
### 済（コミット済み・`npm run tokens:build` 実行済み）
...
0. **`disabled` の行き先を決める**（`check:contrast` を通す前提条件）。`gy9-5`（`#f5f5f5`）へ上げるとページに対して 1.16:1、`gy7-5`（`#b6b6b6`）へ下げると 1.61:1 だが無効状態がかなり濃くなる。`neutral` intent の塗りでもあるので、無効コントロールの見た目と intent の見た目が同時に動く点に注意
1. **`Alert` の透過をどうするか判断**（未判断・ユーザー）。透過のまま 1.21 で通すか、不透明にして 1.26 まで上げるか。`backdrop-filter` は無意味ではない（単色上でも 5.49%、模様上では 99.70% のピクセルが変わることを実測済み）ので、重なり表現とのトレードオフ
2. **衝突箇所の洗い出し**。`border-secondary`（37 ファイル）と `disabled`（18 ファイル）は**どちらも `gy8-5` を指しており、新しいページ色と同じ**になった。ページの上に直接置かれた場合だけ消える性質なので、**理屈で 37 ファイルを推測せず、レンダリングして実際に消えている箇所を拾う**方針にしてある
3. **受け入れ条件の実証**: `Alert intent="default"` から `box-shadow` を外して面が判別できること。現状 `alert.module.scss:12` は `border: ... solid transparent` で、**輪郭を影だけが担っていることをソースで確認済み**
4. **VRT ベースライン全面撮り直し**（どの結論でも必要）
