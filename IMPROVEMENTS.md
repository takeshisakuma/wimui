...
| CI-1 | `[skip ci]` head 問題（品質ゲートのすり抜け） | **VRT update のコミットバックが `[skip ci]` 付きで最終 head になると、その head で品質ゲート（Lint/audit:lib/check:tokens 等）が一度も走らないまま「緑」に見える**。2026-07-18 に T10（PR #27）でまさに顕在化: letter-spacing トークン追加を `token-snapshot.json` に反映し忘れたが、`[skip ci]` ベースラインが最終 head だったため Lint がスキップされ、赤が main に入り次の PR #29 で初めて検出（#29 で修正）。加えて同日、**push イベントの取りこぼし**で PR head に CI が起動しない事象も観測（空コミットで再トリガーして回避）。**重要な技術的事実**: コミットバックは `GITHUB_TOKEN` push のため GitHub の再帰防止仕様で**どのみち workflow を再起動しない**（∴「`[skip ci]` を外せば Lint が再実行される」は誤り。PAT を使わない限り head で自動再検証は不可能） | **一部着手**（2026-07-18。**①実装済**: `vrt.yml` に `update-gate` ジョブ（tsc / eslint / stylelint / check:imports / audit:lib）を追加し、`commit-snapshots` を `needs: [vrt, update-gate]` に変更。これで**ベースライン更新フロー内でゲートを検証**し、通らなければベースラインをコミットしない＝T10 型の drift を含む変更は update run が赤になり、ベースラインすり抜けを構造的に阻止（GITHUB_TOKEN 非依存）。**②残（要ユーザー・リポジトリ設定）**: main のブランチ保護＋required checks（Lint & Type Check / Vitest / Smoke bare）。これが push イベント取りこぼし・チェックレス head・赤マージの**構造的**防御（現状 main は保護なし＝赤でもマージ可能）。**③運用**: エージェントはマージ前に `mergeStateStatus` だけでなく **head コミットに緑の required checks が付いているか**を確認する（保護なし環境では CLEAN が無意味なため）） |
