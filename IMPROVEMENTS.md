...
最終更新: 2026-08-03（**0.12.0 と 0.13.0 を公開し、T37 / T38 / T41案② / T47⑤ / T52 / T54 / T56 / T58 / T59 の 9 件を片付けた**。うち T54 / T56 / T58 / T59 は**この日に新しく書いたガードが、その日のうちに実バグを出した**もの。**新規起票は T55〜T60 の 6 件**で、5 件は T32 の 3 枚目（AI アシスタント画面）から出ている）

**明日はここから。順番は決定済み**: ①**T59 の残り 15 件** → ②**T60 の残り（`ChatUI` の狭幅）** → ③**T55** → ④**T57**。T32 の 4 枚目（モバイル前提のレイアウト）はそのあと。**この順にした理由**: 4 枚目でフォーム部品を横並びに置くと T59 の 15 件を踏むので、先に潰しておくほうが手戻りが無い。`ChatUI` の狭幅も 4 枚目で必ず当たる。

**① T59 の残り 15 件**（`npm run check:shrinkable` がラチェット 15 で凍結中）。`width: 100%` と `min-width: 0` が同じ規則にあり、**横並びに置くと 1 文字幅まで潰れうる**形。**実害は未確認**なので、1 件ずつ狭いコンテナでレンダリングして実測し、読める下限を与えるか `shrinkable-ok` で逃がす（T58 と同じやり方）。内訳: form 10（`Select`×2 / `Transfer`×2 / `Textarea` / `SmartSearchInput` / `RichTextEditor` / `QueryBuilder` / `OtpInput` / `InputBase`）／ typography 2（`Label` / `Code`）／ overlay 1（`Menu`）／ ai 2（`ModelSelector` の 33 行目 / `MarkdownRenderer`）。**`InputBase` と `Label` は多くのフォーム部品の土台なので影響が広い**。**フィルタ帯に `Select` や `Textarea` を横並びに置くのは実アプリで普通の形**なので、そこから測るのが早い。
...
**2026-08-03 時点の順番（決定済み）**: ①T59 の残り 15 件 → ②T60 の残り（`ChatUI` の狭幅）→ ③T55（VRT が外部画像に依存）→ ④T57（`AppShell` の Composition API）→ ⑤T32 の 4 枚目。理由は冒頭の「明日はここから」を参照。
