...
| T265 | **`Marquee` はホバーでしか止められない（キーボード・タッチでは止める手段が無い）** | **P2** | 起票 2026-09-21（T251 ⓑ で用途を書くために実装を読んで発覚）。`pauseOnHover`（既定 true）が唯一の停止手段で、フォーカスでもタップでも止まらない。WCAG 2.2.2（動き続ける内容には一時停止・停止・非表示の手段が要る）はポインタ以外の利用者で満たせない。`prefers-reduced-motion` は `base.scss` / `reset.scss` の全体規則がアニメーションを 1 回で止めるので拾えている。<br>**やること**: フォーカスが中に入ったら止める（`:focus-within`）か、明示的な一時停止ボタンを置くかを決める。 | — |
