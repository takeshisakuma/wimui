...
**明日はここから。やることは 1 つ: T32 の 4 枚目（モバイル前提のレイアウト）。**
...
**4 枚目で当たると分かっているもの**:
- **T60-② は未解決のまま残っている** —`ChatUI` の狭幅。390px で `messageList` 204px → `message` 150px → `bubble` 144px → `CodeBlock` 124px → `pre` **122px**。削っているのは `.message` の `max-width: 80%` とアバターで、**T61 の下限では直らないことを実測済み**。モバイル前提の画面なら正面から当たるので、**そこで設計判断（狭幅でアバターを畳む・バブルの余白を詰める）をするのが妥当**。
