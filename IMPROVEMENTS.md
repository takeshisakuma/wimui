...
最終更新: 2026-09-21（その 100・**セッションを閉じた状態**）（<br>**■ 引き継ぎ状態**: **開いている PR は 0 件。0.31.0 を公開した。** この回の後半に 12 本が main に着地した（#673 その 99 の引き継ぎ / #674 T250 ③④ / #675 T251 / #676 運用メモの移設 / #677 `AGENTS.md` の 5 節の移設 / #678 T265 / #679 品質ゲートの正本 / #680 0.31.0 の changeset / #681 Version Packages / #682 その 100 の引き継ぎ / #683 プローブ残件の退避 / #684 changeset の付け忘れの警告）。**台帳の内訳は 未完了 0 / 判断待ち 0 / 保留 3 / 対応予定なし 2** ── こちらから進められる行は無い（T23 / T201 は上流待ち、T258 は再発待ち、T207 / T237 は対応予定なし）。<br>**■ 0.31.0 を公開し、4 つとも実物で確かめた。** ①npm: 版指定の取得が 200・`latest` = 0.31.0・provenance（SLSA）つき ②タグ `v0.31.0` が Version PR のマージコミット `6545b2c4` を指す ③GitHub Release が公開済み（下書きではない）④**公開された tarball を展開して**、`StatusContent` の `data-align`・`Stats.Trend` の `data-intent`・改名した `--wim-comp-trend-success-fill`・`Marquee` の `inert` と `:focus-within`・同梱の `llms.txt` が v0.31.0、を確認した。Version PR の head には CI が付いた（bot の head でもラン 0 件にならなかった）。<br>**■ この回にやったこと。** **T250 ③④**（`EmptyState` に `align` を足し既定を左揃えへ＝破壊的変更 / `Stats.Trend` に `intent` を足し色を向きから分離 / 合成ルール 11 を「Trend を使うな」から「`intent="danger"` を付ける」へ）/ **T251**（`Dashboard` の `columns` の選び方、`FAQSection` / `Marquee` の用途を MDX に。ⓒⓓ は対応予定なし）/ **T265**（`Marquee` をフォーカスで止める）/ **置き場の基準の残り**（「運用メモ」→ `docs/rules/ci-and-guards.md`、`AGENTS.md` 231 → 156 行、品質ゲートの正本 → `docs/rules/quality-gates.md`）/ **changeset**（0.31.0）。<br>**■ 作業の途中で見つけた欠陥 5 件（どれも赤が出ていなかった）。** ①**`EmptyState` に渡した `id` / `aria-*` が捨てられていた**（`StatusContent` が知らない props を受け取らなかった）②**`Marquee` の複製が支援技術に `repeat` 回読まれ、中のリンクに Tab で `repeat` 回止まっていた** ③**品質ゲートの一覧が 3 か所（`AGENTS.md`・PR テンプレート・`scaffold` の出力）にあり、3 つとも中身が違っていた** ④**0.30.0 以降に changeset 無しで入った利用者向けの変更が #674 のほかに 2 本**（#657 `Carousel` / #659 `Dropdown`）⑤pt の `stats_revenue` が英語のまま。<br>**■ 測って分かったこと。** ①**VRT の撮り直しはファイルの大きさでは仕分けられない** ── 揃えだけ変わった `EmptyState` の 18 枚は全部 100 バイト以下の差で、バイトで分けていたら意図した変更をノイズに入れていた。**repo にある `scripts/vrt-diff-report.js <SHA>` で画素で分ける**（#674 は新規 4 / 意図した変更 18 / ノイズ 95 / 常連の小さな揺れ 5 ＝説明できない枚数 0）②**`AGENTS.md` の 5 節は写しではなかった** ── 命名規則・「SCSS を import しないとスタイルが一切当たらず、テストでも lint でも検出できない」罠・置き場は移動先に無かったので、削らずに移した ③「運用メモ」のリリース手順の節は古い観察だった（Version PR で走るチェックを 5 本と書いていた。実測は 23 件）。<br>**■ この回の誤り 5 件。** ⓐ**a11y の説明で HTML 属性の `inert` をバッククォートで囲み、docs の乖離検査が prop 名と読んで落ちた**（手元で `audit:docs` を通していなかった。**docs を触ったら `audit:docs` を丸ごと流す**）ⓑVRT をバイトで仕分けていた（上の①）ⓒ`node -e` の中の `\d` が食われた（3 回目。長いものは Write でファイルへ）ⓓCRLF のテストファイルに LF 前提の目印で差し込もうとして一致しなかった（Edit を使う）ⓔ移動の欠落検査が、意図して置き換えた索引の行を「欠落」と出した（検査がファイルを書いた後に走る作りで、結果は正しかった）。<br>**■ 運用を 1 つ変えた（重要）: changeset は PR ごとに同梱する**（#684・ユーザー判断）。以前はリリース直前にまとめて書く運用が実態で、0.31.0 では #657 / #659 を取りこぼした（過去 1 か月の main では、出荷物に触れた 44 コミット中 29 本が PR の時点で changeset を持っていなかった）。**`src/`（テストを除く）/ `tokens/` を変えた PR に changeset が無いと `changeset-reminder` が PR にコメントする**（警告のみ・ジョブは落とさない。版を上げない変更は `npx changeset --empty`）。数週間様子を見て、誤検出が空の changeset でさばけると分かったら落とす形に上げる（ユーザー判断）。`AGENTS.md` の委任ポリシーを `RELEASING.md`（「changeset の作成と PR まではエージェントが行ってよい」）にそろえた。<br>**■ 次の入り口。** ①**月曜 09:00 JST の最初の定期点検 issue**（本番経路は dry_run で未実走。立ったこととラベル `maintenance` を確かめる）②**changeset の警告の様子見**（誤検出が出たら記録し、落とす形に上げるかをユーザーに諮る）③Dependabot の週次（定期点検 issue に出る）。（<br>**その 99 以前の申し送り**は `docs/history/improvements-archive.md`。）
...
**2026-09-21（その 100・終業時点）の順番**: ①**月曜の定期点検 issue を確かめる** → ②**定期点検の中身を回す**（Dependabot・ブロック中の依存・隔週の項目）→ ③**changeset の警告の様子見**（数週間。誤検出を記録する）。0.31.0 は公開済み。台帳にこちらから進められる行は無い。

**着手前に読むもの**: タスクを済にしたら同じ PR で `npm run improvements:archive`。起票の番号は `npm run check:improvements` の「次に振る番号」（いま T266 / CI-13）。**docs を触ったら `npm run audit:docs` を丸ごと流す**（部分のガードだけでは乖離検査を通せない）。**VRT の撮り直しは `node scripts/vrt-diff-report.js <SHA>` で画素で仕分ける**。**出荷物（`src/` / `tokens/`）を変える PR には changeset を同梱する**（版を上げない変更は `npx changeset --empty`。無いと `changeset-reminder` がコメントする）。
