...
| # | 改善 | 内容 | 状態 |
|---|---|---|---|
| T267 | **部品の質感と動きが、どの slop 基準にも映っていなかった**（ホバーの浮き上がり・弾む easing・既定のすりガラス・blur へのトークン流用） | 起票 2026-09-26（ユーザーの「デザイン面で AI slop を減らせる点は？」から）。T250 / T251 / #689 で部品の既定（intent・variant・揃え・斜めグラデ）は採点の対象になったが、`check:slop` が部品 SCSS で見ているのは斜めグラデだけで、**質感と動きは 1 行も見ていなかった**。読むと既定に埋まっていた: solid の `Button` のホバーが「浮く＋`shadow-lg`＋光沢＋`out-back` で弾む」の 4 重／`TreeView` / `Alert` / `Banner` / `Toast` / `Snackbar` が**既定で**すりガラス（禁止表「量産グラスを既定にしない」）／`Card` / `Badge` / `Chip` / `Tag` / `InteractiveArea` / `Lightbox` / `Gallery` がホバーで浮く・拡大する／`blur(var(--wim-radius-sm))`・`blur(var(--wim-spacing-lg))` のように**別カテゴリのトークン**を blur に流している（`check-hardcoded-values.js` は `\d+px` の綴りを探すので届かない＝T266 と同じ形）。**① ガード**: `check:component-slop`（4 種を見る。件数で凍結せず `component-slop-ok:` の注記を 1 件ずつ要求）と `prove:component-slop`（全量 / lint-staged の絶対パス / `--probe` の 44 経路）。弾む曲線は**名前でなく曲線で**判定する（最初の版は `out-back` の名前だけを見ていて、同じ種類の `--wim-transition-spring`（`FloatButton` / `Image`）と `out-back` を包んだ `--wim-transition-entrance`、`Lightbox` の `cubic-bezier(0.34, 1.56, …)` の直書きを素通りしていた）。値が同じトークンへ寄せられる blur 5 件はここで直した（Sidebar / BottomSheet の `radius-sm`→`blur-2xs`、TreeView / Snackbar / FloatButton の生の `8px` フォールバック）。残りは `T267-②〜④ で外す` の注記つきで残し、下の PR で注記ごと消す。**② Button** のホバーの合図を色だけにする（浮き上がり・影はテーマが `--wim-button-transform` / `--wim-button-shadow-hover` で選んだときだけ）。**③ 既定のすりガラス**を外す（ページ内の面はサーフェス階層トークンへ。`Alert` の blur は T45 で「画素が変わる」ことを根拠に残したが、それは効果の有無で、要るかどうかではない）。**④ ホバーの合図を 1 つに絞る**（`Card` の outline は枠の色と浮き上がりの 2 重）。**理由つきで残したもの**: 浮いている部品（`FloatButton` / `BackTop`）の浮き上がり、写真・動画の上の操作（`Lightbox` / `Video`）のすりガラス、ドラッグ中（`SortableList`）、`Rating` の星と `CalendarHeatmap` のセルの拡大 | **P2**（① #696 / ② #697 / ④ #699 / ③ #698 レビュー中（VRT のベースライン更新あり）/ `Gallery` のサムネイルの拡大は**判断待ち**: ホバーの合図がそれしかない。見た目が変わるので PR ごとに VRT と `judge:slop` を添える） |
