...
**新規の公開コンポーネントはカタログの単体だけでは出荷しない。** 必ず一度合成する（T179 のプローブ。`RULES.md`）。確認が終わったら画面は捨て、部品の修正と Realistic な単体ストーリーを残す。カバー率のために Pattern ページへ書かない。
