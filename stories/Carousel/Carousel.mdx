...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td><strong>タッチ操作（スワイプ）</strong></td>
      <td>モバイル環境では、「横にスワイプしてスライドさせる」直感的なタッチドラッグ操作に対応しています。一方で、PC（マウス操作）環境では、無意識な画像・テキストのブラウザ標準選択（ドラッグ保存など）を防ぐため、敢えてマウスドラッグによるスライド操作は実装せず、矢印ボタンやインジケーターのみでの操作としています。</td>
    </tr>
    <tr>
