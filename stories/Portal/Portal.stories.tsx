                ...
                <Card variant="outline" padding="sm">
                    <Stack gap="xs">
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                            <strong>{name}</strong>
                            <Badge content={active ? "稼働中" : "停止中"} color={active ? "success" : "neutral"} size="small" />
                        </div>
                        <Button
                            size="small"
        ...
        return (
            <div style={{ width: "100%", maxWidth: "800px" }}>
                ...
                <div className="notification-grid">
                    <Stack gap="md">
                        <h4>制御パネル</h4>
                        <p style={{ fontSize: "14px", color: "#666" }}>各コンポーネントの状態変更が、右側のモニタリングエリアに Portal 経由で通知されます。</p>
                    ...
                    <Card variant="flat" style={{ background: "#1e293b", color: "#f8fafc", minHeight: "400px", display: "flex", flexDirection: "column" }}>
                        <Card.Header style={{ borderBottom: "1px solid #334155", color: "#94a3b8" }}>
                            <Stack direction="row" justify="between" align="center">
                                <span style={{ fontSize: "12px", fontWeight: "bold" }}>SYSTEM MONITOR</span>
                                <Badge content="LIVE" color="error" size="small" />
                            </Stack>
                        </Card.Header>
                        <Card.Body style={{ flex: 1, overflowY: "auto", padding: "12px" }}>
                            <div ref={setLogContainer} style={{ marginBottom: "16px", borderBottom: "1px dashed #334155", paddingBottom: "16px" }}>
                                {/* ここに Portal からの「稼働中メッセージ」が表示される */}
                            </div>
                            <div style={{ fontSize: "11px", fontFamily: "monospace" }}>
                                <div style={{ color: "#64748b", marginBottom: "8px" }}>--- イベント履歴 ---</div>
                                {logs.map(log => (
                                    <div key={log.id} style={{ marginBottom: "4px", color: log.type === "success" ? "#4ade80" : log.type === "warning" ? "#fbbf24" : "#94a3b8" }}>
                                        {`> ${log.msg}`}
                                    </div>
                                ))}
                                {logs.length === 0 && <div style={{ color: "#475569" }}>待機中...</div>}
                            </div>
                        </Card.Body>
                    </Card>
                </div>
...
export const SidePanelDetail: Story = {
    render: () => {
        ...
        const tasks = [
            { id: 1, title: "コードレビュー", detail: "新しいPortalストーリーの実装を確認し、アクセシビリティチェックを行う。" },
            { id: 2, title: "バグ修正", detail: "テーブルのレスポンスが遅い問題を調査し、メモダイズを適用して最適化する。" },
            { id: 3, title: "ドキュメント作成", detail: "MDXファイルに具体的なユースケース（通知センターとサイドパネル）を追加する。" },
        ];
            ...
            return (
                ...
                >
                    <Stack direction="row" justify="between" align="center">
                        ...
                        <Icon name="ChevronRightIcon" style={{ color: isSelected ? "#3b82f6" : "#cbd5e1" }} />
                    </Stack>
                    ...
                    {isSelected && (
                        <Portal container={container}>
                            <div style={{ animation: "fadeInUp 0.3s ease-out" }}>
                                ...
                                <Badge content={`TASK-00${task.id}`} variant="outline" size="small" style={{ marginBottom: "12px" }} />
                                <h3 style={{ margin: "0 0 16px 0" }}>{task.title}</h3>

                                <Card variant="flat" style={{ background: "#f8fafc", marginBottom: "20px" }}>
                                    <p style={{ margin: 0, fontSize: "14px", lineHeight: "1.6", color: "#334155" }}>{task.detail}</p>
                                </Card>

                                <Stack gap="xs">
                                    <label style={{ fontSize: "13px", fontWeight: "bold", color: "#64748b" }}>担当者メモ:</label>
                                    <Textarea
                                        value={note}
                                        ...
                                        fullWidth
                                    />
                                    <p style={{ fontSize: "11px", color: "#94a3b8" }}>※リスト上の別のタスクに切り替えても、再選択すれば入力内容は保持されています（コンポーネントが破棄されていないため）。</p>
                                </Stack>
                            </div>
                        </Portal>
                    )}
                ...
                <Card variant="outline" padding="none" className="portal-demo-card">
                    ...
                    <div className="portal-demo-content">
                        <div style={{ padding: "20px", borderBottom: "1px solid #e2e8f0", background: "#f8fafc", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                            <span style={{ fontSize: "14px", fontWeight: "bold", color: "#64748b" }}>詳細プレビュー</span>
                            {selectedId && <Button size="small" label="閉じる" priority="secondary" onClick={() => setSelectedId(null)} />}
                        </div>
                        <div ref={setPanelContainer} style={{ flex: 1, padding: "30px", overflowY: "auto", position: "relative" }}>
                            {!selectedId && (
                                <div style={{ height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", color: "#94a3b8", textAlign: "center" }}>
                                    <Icon name="InfoCircleIcon" style={{ width: "48px", height: "48px", marginBottom: "12px", opacity: 0.5 }} />
