...
1. **ガードは「故意に違反を作って落ちること」を実証してから完成とする。**  
   未実証のガード、および「0 件」という結果は信用しない。同日の実例: `check:slop` のラチェットは lint-staged 経由だと**部分集合を全体基準と比べるため常に素通り**していた（故意に px 直書きを入れて初めて判明）。`check:llms` は**バイト厳密比較のせいで Windows でだけ落ちる**偽陽性だった（CRLF が 46 行混入。git 的には差分ゼロ）。T33 の検出スキャナは既知ケース（`Card` の `padding="xl"`）を拾えるか自己検証できず結果を破棄した。**検出すべき既知の事例を過去のコミットで再現して鳴らすこと**が完成条件。
