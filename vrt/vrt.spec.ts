      ...
      for (const story of stories) {
        ...
        test(`${story.title} - ${story.name}`, async ({ page }) => {
          ...
          await expect(page).toHaveScreenshot(`${theme}/${story.id}.png`, {
            ...
            maxDiffPixels: 50,
            /**
             * **T238 の実験。誤検出の数を測るための枝で、このままマージしない。**
             *
             * 既定の pixelmatch は `includeAA: false` でアンチエイリアス画素を
             * 除外する。ソート記号のような細い字形は**ほぼ全体が AA の縁**なので、
             * 面積を数える前に消える ── 実測で **生の差分 75 画素に対し
             * Playwright が数えたのは 9 画素**だった（#564 が緑で通った理由）。
             *
             * `ssim-cie94` は AA 除外を通さない。ローカルで #564 の 1 行を戻した
             * 対照に当てると、**実際に変化した 10 ストーリー × 2 テーマ = 20 件だけ**が
             * 落ち、変化していない 6 ストーリーは通った（既定の比較器は 32/32 緑）。
             *
             * ここで測りたいのは**鳴ってはいけない経路で鳴らないこと**。
             * 変更を含まないこの枝で全 2186 テストが緑なら誤検出ゼロ。
             */
