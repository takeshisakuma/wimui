      ...
      for (const story of stories) {
        ...
        test(`${story.title} - ${story.name}`, async ({ page }) => {
          ...
          await expect(page).toHaveScreenshot(`${theme}/${story.id}.png`, {
            ...
            maxDiffPixels: 50,
            /**
             * **既定の pixelmatch をやめて `ssim-cie94` にする（T238、2026-08-30）。**
             *
             * **上の 2 つの注記だけでは塞げない 3 つ目の盲点があった。** #564 は
             * 6 つの DataGrid ストーリーから見出しのソート記号を消したのに、
             * VRT compare は緑でベースラインも 1 枚も更新されずにマージされた。
             *
             * **原因は面積でも色閾値でもなく、`includeAA: false` だった。**
             * #564 の 1 行を戻した使い捨てブランチで `maxDiffPixels: 0` にして
             * Playwright が実際に数えた値を読むと、**生の差分 75 画素に対し 9 画素**。
             * **66 画素がアンチエイリアスとして捨てられていた**（生の 75 は
             * `maxDiffPixels: 50` を超えているので、面積では説明がつかない）。
             * ソート記号のような細い字形・アイコンは**ほぼ全体が AA の縁**なので、
             * 面積を数える前に消える。
             *
             * **:146 の「AA 画素は数えられないから閾値を下げても安全」は、同じ性質の
             * 裏返しだった** ── 安定性の根拠が、そのまま細い記号を見えなくしていた。
             * **だから `maxDiffPixels` を下げても解けない**（9 未満にしないと鳴らず、
             * そこまで下げれば全面フレーク化する）。**差分の密度で見る案も効かない**
             * （密度を測る前に画素が消えている）。
             *
             * 受け入れ条件の実証:
             * ①**既知の事例で鳴ること** ── #564 の 1 行を戻した対照に当てると、
             *   **実際に変化した 10 ストーリー × 2 テーマ = 20 件だけ**が落ち、
             *   変化していない 6 ストーリーは通った（既定の比較器は 32/32 緑）。
             * ②**鳴ってはいけない経路で鳴らないこと** ── ソースを 1 行も変えない枝で
             *   CI の**全 2186 テストが緑**（誤検出ゼロ）。
             * ③**代償** ── 1 シャード 518 テストで **5.4m → 6.1m（+13%）**。
             *
             * **`_comparator` は Playwright の非公開オプション**（アンダースコア始まり）。
             * 上げるときに消える可能性があるので、**Playwright の major 更新では
             * ①の対照をもう一度回すこと**。
             */
