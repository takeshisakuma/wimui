...
<Meta title="Category/Selection Controls" />
...
<table>
  ...
  <tbody>
    <tr>
      <td><strong>Switch / SwitchGroup</strong></td>
      <td>1つ (ON/OFF)</td>
      <td>-</td>
      <td>○</td>
      <td><strong>設定を即座に反映させたい</strong></td>
      <td>Wi-Fi, ダークモード, 通知設定の即時変更</td>
    </tr>
    <tr>
      <td><strong>Checkbox / CheckboxGroup</strong></td>
      <td>単一 または 複数</td>
      <td>少数 (2〜10)</td>
      <td>×</td>
      <td><strong>「保存」や「送信」ボタンで確定させたい</strong></td>
      <td>設定のオンオフ、利用規約同意、複数フィルタ</td>
    </tr>
    <tr>
      <td><strong>Radio / RadioGroup</strong></td>
      <td>単一</td>
      <td>少数 (2〜5)</td>
      <td>×</td>
      <td><strong>少数の候補から1つだけ選び、後で送信したい</strong></td>
    ...
    <tr>
      ...
      <td>少数 (2〜5)</td>
      <td>○</td>
      <td><strong>表示モードやフィルタをその場で切り替えたい</strong></td>
    ...
    <tr>
      ...
      <td>中〜多数 (5〜20)</td>
      <td>×</td>
      <td><strong>場所を取らずに、中規模のリストから選びたい</strong></td>
    ...
    <tr>
      ...
      <td>多数 (20〜)</td>
      <td>×</td>
      <td><strong>膨大なデータから検索して絞り込みたい</strong></td>
    ...
    <tr>
      ...
      <td>中〜多数</td>
      <td>×</td>
      <td><strong>複数の項目をリストから選びたい</strong></td>
      <td>タグ付け、参加者選択</td>
    </tr>
    <tr>
      <td><strong>Chip / ToggleGroup</strong></td>
      <td>単一 / 複数</td>
      <td>少数</td>
      <td>○</td>
      <td><strong>視覚的に強調されたボタン形式で選びたい</strong></td>
...
<table>
  ...
  <tbody>
    <tr>
      <td><strong>ラジオボタン (Radio)</strong></td>
      <td>常に1つだけ選択可能です。いずれかの項目が必ず選択されている必要があります。</td>
    </tr>
    <tr>
      <td><strong>チェックボックス (Checkbox)</strong></td>
      <td>0個以上の複数選択が可能です。各項目が独立しており、どれも選ばないという選択も可能です。</td>
    </tr>
    <tr>
      <td><strong>スイッチ (Switch)</strong></td>
      <td>2つの状態（オン/オフ）を切り替えます。設定の有効化・無効化という「バイナリな状態」を示すのに最適です。</td>
    </tr>
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    <tr>
      <td><strong>即時反映 (Immediate)</strong></td>
      <td>スイッチや Segmented Control は、操作した瞬間に設定が保存されたり画面が切り替わったりする場合に使用します。</td>
    </tr>
    <tr>
      <td><strong>確認後に反映 (Deferred)</strong></td>
      <td>ラジオボタンやチェックボックスは、通常「保存」や「送信」ボタンを押して初めて状態が確定するフォーム形式で使用します。</td>
    </tr>
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>ラジオボタンは「通常配送」などの推奨値をあらかじめ選択しておくのが一般的です。チェックボックスは基本「未選択」をデフォルトとします。</td>
    </tr>
    <tr>
      <td><strong>レイアウト</strong></td>
      <td>選択肢は横に並べるよりも、<strong>垂直に並べる</strong>ほうが視線の移動がスムーズで、モバイルでのタップミスも減らせます。</td>
    </tr>
    <tr>
      <td><strong>アクセシビリティ</strong></td>
      <td>ラベルテキスト自体もクリック（タップ）可能にし、ヒットエリアを十分に確保してください。</td>
    </tr>
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>フィルター条件（色・サイズ）、通知設定（メルマガ/新機能）、利用規約の同意</td>
    </tr>
    <tr>
      <td><strong>Switch</strong></td>
      <td>Wi-FiやBluetoothのON/OFF、ダークモードの切り替え、通知の即時有効化</td>
    </tr>
    <tr>
      <td><strong>Segmented Control</strong></td>
