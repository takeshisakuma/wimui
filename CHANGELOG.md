...
- 27d4998: キーボードのフォーカス表示が見えにくかった部品を直しました。
...
- fd10fb6: レイアウト部品の既定が「AI が置いたまま」に見えていた 3 点を、画面側ではなく既定と API の側で直しました。
...
- 199ee70: `EmptyState` の既定の揃えを左へ変え、`Stats.Trend` / `Carousel` / `Dropdown` に prop を足した。**`EmptyState` は素で置いたときの見た目が変わるため、0.x の慣例に従って minor に載せる。**
...
- 93f82cf: 既定値を「静かなほう」へ寄せ、型はあるのに効いていなかった `BentoGrid` の `span` を実装した。**素で置いたときの見た目が変わるため、0.x の慣例に従って minor に載せる。**
...
- 557f715: `ScheduleView` の optional peer（`@fullcalendar/*`）のレンジを `>=6.0.0` から `^6.0.0` に狭めた。**いままで FullCalendar 7 を許容すると宣言していたが、7 では動かない。**
...
- d049546: `ScheduleView` の自前スタイルを、他と同じ `@layer component` の中へ入れた（T243 の最終弾）。**これで全 212 個のスタイルシートがカスケードレイヤーの中に入った。**
...
- ec49659: `MarkdownRenderer` と `StreamingText` のスタイルを、他と同じ `@layer component` の中へ入れた（T243 の第 7 弾）。
...
- 0166ab5: `Dashboard` のスタイルを、他と同じ `@layer component` の中へ入れた（T243 の第 6 弾）。
...
- f5ec3fd: `NodeGraph` の自前スタイルを、他と同じ `@layer component` の中へ入れた（T243 の第 5 弾）。
...
- 2d9168b: `InteractiveGraph` の自前スタイルを、他と同じ `@layer component` の中へ入れた（T243 の第 4 弾）。
...
- 407c92d: `DataGrid` のスタイルを、他と同じ `@layer component` の中へ入れた（T243 の第 3 弾）。
...
- 6b4866c: `List` のスタイルを、他と同じ `@layer component` の中へ入れた（T243 の第 2 弾）。
...
- c1fee1b: 25 コンポーネントのスタイルを、他と同じ `@layer component` の中へ入れた（T243 の第 1 弾）。
...
- 988da74: `Table` / `DataGrid` の見出し行まわりを 3 点直した。
...
- 5983a72: ラベルの文字を持たない `Checkbox` / `Radio` / `Switch` が、置かれた場所の中心から左へずれていたのを直した。
...
- 54a0530: `Barcode` を追加した（`wimui` / `wimui/data-display`）。Code 128 と EAN-13 の
...
- 176d83b: 在席を示す `Presence` を `data-display` に追加した（T225 ④）。ルートの `wimui` バレルと `wimui/data-display` の両方から使える（optional peer に依存しない）。
...
- 1ec2017: 同梱している `llms.txt` / `llms-full.txt` が、docs サイトと違う説明文を 9 部品ぶん載せていたのを直した（T231）。
...
- 77eb959: チャート 10 件（`AreaChart` / `BarChart` / `FunnelChart` / `GaugeChart` / `Heatmap` / `LineChart` / `PieChart` / `RadarChart` / `ScatterChart` / `Treemap`）に、アクセシブル名と値のテキスト代替を入れた（T230）。
...
- 488796e: `Footer.Section` と `KeyboardShortcuts` に `titleLevel` を足した。**渡したときだけ** `title` が見出し（`h2`〜`h6`）として描かれる。
...
- 68ca30e: `Kanban` の列・`ContextMenu` のグループ・`CommandPalette` のグループが、**支援技術から名前のある区切りとして辿れるようになった**。
...
- 02367fb: solid のボタンで、**面の上に乗るシーンまで含めると 4.5:1 を割っていた** 2 つの色を一段深いトーンに差し替えた。
...
- 723ad71: `ImageCompare` を `orientation="vertical"` で使ったとき、**「後」側のラベルが見えなくなっていた**のを直した。
...
- 98ca3a0: チャートの**軸の目盛り**と**凡例のラベル**の文字色を、非テキスト用に設計された色からテキスト用トークンに変えた。
  ...
  **凡例のラベル**: recharts の `<Legend>` は既定でラベルの文字色に**系列色**を使う。系列パレットは「細いマークが見分けられること」＝ 3:1 を前提に設計してあるので、文字に使うと構造的に 4.5 へ届かない（light の secondary で 2.02）。色を選び直しても解けない（3:1 を満たす彩度のある色はたいてい 4.5 を割る）ため、**文字色だけを `--wim-color-text-secondary` に寄せ、色による系列の識別は凡例のスウォッチに任せる**ようにした。
  ...
  あわせて**軸線と目盛り線の色**も上げた（`--wim-color-text-disabled` → `--wim-color-text-tertiary`）。線は非テキスト（WCAG 1.4.11 の 3:1）だが、**チャートは自分の背景を塗らない**ので、Card に入れずページ地に直接置くと light で 2.74 まで落ちていた。この 2 つの間に 3:1 を通るトークンは無く（border 系はすべて届かない）、通るうちで一番軽いものを採っている。

  **グリッド線と系列の色は変えていない。** グリッド線は読み取りの補助で、無くてもデータは読める＝装飾のため。

  `AreaChart` / `BarChart` / `LineChart` / `PieChart` / `RadarChart` / `ScatterChart` / `Heatmap` の見た目が変わる ── **目盛りと凡例の文字、および軸線が濃くなる**。公開 API に変更は無い。
...
- b514144: 素の見出し（`h1`〜`h6`）の行間を、本文用の比率ではなく見出し用のトークンで決めるようにした。
...
- 3ee6e8c: `SpeedDial` のアクション `intent` が内側の `FloatButton` に届くようにしました。型にはもともとあり、渡しても黙って捨てていたのが欠陥です（T173）。
...
- bed617d: `Gallery` の `selectable` で、写真クリックが選択になって `onItemClick` が届かない穴を直しています。チェックで選び、写真クリック（と Enter）は `onItemClick` です。Space はチェックのキーボード相当。Shift 範囲選択はチェック側に残しています。
...
- 8b92ad0: `Terminal` の長い行が単語の途中で折り返さなくなりました。実端末と同じく、狭い幅では内部で横スクロールします。

## 0.23.2

### Patch Changes

- 19e1408: `NodeGraph` がダークテーマに追随するようになりました。**それまでダークではノードのラベルが 1 文字も見えていませんでした。**
...
- 9c89ef1: `NodeGraph` と `InteractiveGraph` の辺が、地に対して読める濃さになりました。
...
- bc56629: `ArtifactsOverlay` が import できるようになりました。併せて狭幅の欠陥を 2 件直しています。
...
- d028b49: `SignaturePad` and `Rating` stop stretching to the width of the field they sit in.
...
- d028b49: `Treemap` tiles that are too narrow for their name now show a shortened one instead of none.
