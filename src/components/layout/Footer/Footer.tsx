...
export interface FooterSectionProps extends React.ComponentPropsWithoutRef<"div"> {
   ...
   *
   * **既定を見出しにしてはいけない**（T191 の実績）── かつて `h4` 固定だった頃、
   * ページに `h1` / `h2` があると段が飛んで axe の `heading-order` が鳴った
   * （`Probes/SaginomiyaPool` で実際に出た）。**フッタの欄名は文書の見出しでは
   * ない**というのがそのときの判断で、ここはそれを踏襲する。
   *
   * ただし「欄名を見出しとして辿らせたい」ページも実在するので、**段を明示した
   * ときだけ**見出しになるようにしてある。`Alert` の `titleTag`（既定 `div`）と
   * 同じ形。段を決めるのは**ページ側の構造を知っている呼び出し元**の仕事。
   */
