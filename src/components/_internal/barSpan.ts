...
export const spanPixel = (
  ...
  if (range === 0) return rect.y;
  return rect.y + ((high - value) / range) * rect.height;
};
...
export const spanDomain = (
  ...
  const range = max - min;
  // 値幅が 0（全部同じ値）のときは比で余白を作れないので、絶対値から作る。
  const pad = range > 0 ? range * ratio : Math.max(Math.abs(max) * ratio, 1);

  // 丸めの刻み。値幅の 1 桁下（幅 390 なら 10、幅 4.9 なら 0.1）。
  const magnitude = range > 0 ? range : Math.abs(max) || 1;
  const step = Math.pow(10, Math.floor(Math.log10(magnitude)) - 1);
  // 掛け算で戻すと 182.8 が 182.80000000000001 になるので、刻みの桁で丸め直す。
  ...
  const low = snap(min - pad, -1);
  const high = snap(max + pad, 1);
  return [min >= 0 && low < 0 ? 0 : low, high];
};
