...
export const Video = ({
    ...
    const [activeMenu, setActiveMenu] = React.useState<"main" | "quality" | "rate" | "fit" | "playlist" | null>(null);

    const activeSrc = qualities && qualities.length > 0
        ? qualities[currentQualityIndex].src
        : (playlist && playlist.length > 0 ? playlist[currentPlayIndex].src : src);

    const activePoster = playlist && playlist.length > 0
    ...
    React.useEffect(() => {
        if (videoId && resumePlayback && videoRef.current) {
            const trackId = playlist && playlist.length > 0 ? `${videoId}-${currentPlayIndex}` : videoId;
    ...
    const handleTimeUpdate = () => {
        if (videoRef.current) {
            ...
            // Save resume time
            if (videoId && resumePlayback && Math.floor(time) % 5 === 0 && duration > 0 && time < duration - 2) {
                const trackId = playlist && playlist.length > 0 ? `${videoId}-${currentPlayIndex}` : videoId;
    ...
    const handleEnded = () => {
        ...
        if (playlist && playlist.length > 0 && autoPlayNext && currentPlayIndex < playlist.length - 1) {
    ...
    React.useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            if (["INPUT", "TEXTAREA", "SELECT"].includes((e.target as HTMLElement).tagName) || !(customControls || advancedControls)) return;

            // Check if focus is within our container or we're in fullscreen
            const isOurVideoFocused = containerRef.current?.contains(document.activeElement) || document.fullscreenElement === containerRef.current;
    ...
    const [lastTapInfo, setLastTapInfo] = React.useState<{ time: number, x: number } | null>(null);
    const [skipIndicator, setSkipIndicator] = React.useState<{ show: boolean, direction: 'forward' | 'backward' }>({ show: false, direction: 'forward' });
    ...
    const handleVideoClick = (e: React.MouseEvent<HTMLVideoElement>) => {
        ...
        if (lastTapInfo && currentTimeMs - lastTapInfo.time < 300) {
            ...
            setTimeout(() => setSkipIndicator({ show: false, direction: 'forward' }), 600);
            setLastTapInfo(null);
        } else {
    ...
    const showNativeControls = customControls || advancedControls ? false : controls;

    const rates = [0.25, 0.5, 0.75, 1.0, 1.25, 1.5, 1.75, 2.0];
    const fitOptions = ["contain", "cover", "fill", "scale-down", "none"] as const;
            ...
            >
                ...
                {skipIndicator.show && (
                    <div className="wim-video-skip-indicator">
                        <Icon name={skipIndicator.direction === 'forward' ? "ChevronRightIcon" : "ChevronLeftIcon"} size="large" />
                        <Icon name={skipIndicator.direction === 'forward' ? "ChevronRightIcon" : "ChevronLeftIcon"} size="large" />
                        <Icon name={skipIndicator.direction === 'forward' ? "ChevronRightIcon" : "ChevronLeftIcon"} size="large" />
                        <span>10秒</span>
                    </div>
                ...
                {(customControls || advancedControls) && (
                    <div className="wim-video-custom-controls" onClick={() => activeMenu && setActiveMenu(null)}>
                        {advancedControls && activeMenu === "main" && (
                            <div className="wim-video-advanced-menu" onClick={e => e.stopPropagation()}>
                                <div className="wim-video-menu-title">
                                    <span>設定</span>
                                </div>
                                <div className="wim-video-menu-items">
                                    {qualities && qualities.length > 0 && (
                                        <button className="wim-video-menu-btn" onClick={() => setActiveMenu("quality")}>
                                            <span>画質</span>
                                            <span style={{ opacity: 0.7 }}>{qualities[currentQualityIndex].label} <Icon name="ChevronRightIcon" size="small" /></span>
                                        </button>
                                    )}
                                    <button className="wim-video-menu-btn" onClick={() => setActiveMenu("rate")}>
                                        <span>再生速度</span>
                                        <span style={{ opacity: 0.7 }}>{playbackRate}x <Icon name="ChevronRightIcon" size="small" /></span>
                                    </button>
                                    <button className="wim-video-menu-btn" onClick={() => setActiveMenu("fit")}>
                                        <span>アスペクト比</span>
                                        <span style={{ opacity: 0.7 }}>{activeFit} <Icon name="ChevronRightIcon" size="small" /></span>
                                    </button>
                                </div>
                            </div>
                        )}
                        ...
                        {advancedControls && activeMenu === "quality" && (
                            <div className="wim-video-advanced-menu" onClick={e => e.stopPropagation()}>
                                <div className="wim-video-menu-title">
                                    <button onClick={() => setActiveMenu("main")} className="wim-video-btn"><Icon name="ChevronLeftIcon" size="small" /></button>
                                    <span>画質</span>
                                </div>
                                <div className="wim-video-menu-items">
                                    {qualities?.map((q, i) => (
                                        ...
                                        >
                                            {currentQualityIndex === i && <Icon name="CheckIcon" size="small" />}
                                            <span style={{ marginLeft: currentQualityIndex === i ? 0 : 24 }}>{q.label}</span>
                                        </button>
                                    ))}
                                </div>
                            </div>
                                ...
                                <div className="wim-video-menu-items">
                                    {rates.map((r) => (
                                        <button
                                            ...
                                            onClick={() => { setPlaybackRate(r); setActiveMenu("main"); }}
                                        >
                                            {playbackRate === r && <Icon name="CheckIcon" size="small" />}
                                            <span style={{ marginLeft: playbackRate === r ? 0 : 24 }}>{r === 1.0 ? "標準" : `${r}x`}</span>
                                        </button>
                                    ))}
                                </div>
                            </div>
                                ...
                                <div className="wim-video-menu-items">
                                    {fitOptions.map((f) => (
                                        <button
                                            ...
                                            onClick={() => { setActiveFit(f); setActiveMenu("main"); }}
                                        >
                        ...
                        {advancedControls && activeMenu === "playlist" && (
                            <div className="wim-video-advanced-menu" onClick={e => e.stopPropagation()} style={{ bottom: 60, right: 16, width: 250 }}>
                                <div className="wim-video-menu-title">
                                    <span>プレイリスト</span>
                                    <button onClick={() => setActiveMenu(null)} style={{ marginLeft: 'auto' }} className="wim-video-btn"><Icon name="CloseIcon" size="small" /></button>
                                </div>
                                <div className="wim-video-menu-items">
                                    {playlist?.map((p, i) => (
                                        <button
                                        ...
                                        >
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                                {currentPlayIndex === i ? <Icon name="PlayIcon" size="small" /> : <div style={{ width: 20 }} />}
                        ...
                        <div className="wim-video-controls-bottom">
                            {playlist && playlist.length > 0 && advancedControls && (
                                <button
                                    ...
                                    onClick={(e) => { e.stopPropagation(); setActiveMenu(activeMenu === "playlist" ? null : "playlist"); }}
                                    aria-label="Playlist"
                                >
                            ...
                            {playlist && playlist.length > 0 && advancedControls && (
                                <button
                                    ...
                                    style={{ opacity: currentPlayIndex >= playlist.length - 1 ? 0.5 : 1 }}
