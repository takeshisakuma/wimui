...
export const Audio = ({
    ...
    // Fades & Playback core
    // ------------------------------------------------------------------------
    const doGainRamp = useCallback((gainNode: GainNode, startVal: number, endVal: number, durationMs: number) => {
        if (!audioCtxRef.current) return;
        const ctx = audioCtxRef.current;
        gainNode.gain.cancelScheduledValues(ctx.currentTime);
        gainNode.gain.setValueAtTime(startVal, ctx.currentTime);
        gainNode.gain.linearRampToValueAtTime(endVal, ctx.currentTime + durationMs / 1000);
    }, []);
    ...
    const pauseActivePlayer = useCallback(() => {
        ...
        if (audioCtxRef.current && activeGainRef.current && fadeOutt > 0) {
            doGainRamp(activeGainRef.current, activeGainRef.current.gain.value, 0, fadeOutt);
            setTimeout(() => {
    ...
    useEffect(() => {
        ...
        if (showMetadata && !title && !artist && !cover) {
            ...
            import("jsmediatags/build2/jsmediatags.js").then((jsmediatagsModule) => {
    ...
    }, [currentTrackIndex, currentTrack, showMetadata, autoPlay, isPlaying, playActivePlayer]);
    ...
    const getNextIndexBase = useCallback((dir: 1 | -1 = 1) => {
    ...
    const playNext = useCallback((dir: 1 | -1 = 1) => {
    ...
    const startCrossfadeToNext = useCallback((nextIndex: number) => {
        if (!activeAudioRef.current || !nextAudioRef.current) return;
        if (!audioCtxRef.current || !activeGainRef.current || !nextGainRef.current) return;
        ...
        <figure className={classNames("wim-audio-container", className)} style={style}>
            ...
            >
                ...
                {customControls && showMetadata && (metaTitle || metaArtist || metaCover) && (
                    <div className="wim-audio-metadata" style={{ position: 'relative', zIndex: 2 }}>
                        {metaCover && <img src={metaCover} alt="Cover" className="wim-audio-cover" />}
                ...
                <div style={{ display: customControls ? "none" : "block", width: "100%" }}>
                ...
                {customControls && (
                    <div className="wim-audio-custom-controls">
                        ...
                        <div className="wim-audio-controls-row">
                                ...
                                {playbackRate && (
                                    <button
                                        ...
                                        onClick={() => {
                                            ...
                                            const nextRate = rates[(rates.indexOf(currentPlaybackRate) + 1) % rates.length];
                            ...
                            <div className="wim-audio-controls-group">
                                ...
                                {sleepTimer && (
                                    <button
                                        type="button"
                                        className={classNames("wim-audio-btn", remainingSleepTime !== null && "active")}
                        ...
                        <div className="wim-audio-controls-row" style={{ justifyContent: 'center', marginTop: '0.2rem' }}>
                            <div className="wim-audio-controls-group">
                                <button type="button" className="wim-audio-btn" onClick={() => playNext(-1)} title="Previous">
                                ...
                                <button type="button" className="wim-audio-btn" onClick={() => playNext(1)} title="Next">
