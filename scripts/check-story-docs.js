 ...
 *
 * `docs/rules/mdx.md`「ストーリー掲載時の注意点（重複排除）」は
 * `<Stories />` と個別 `<Canvas>` の**併用による重複**を戒めているが、
 * **どちらも無い場合**には触れていない。掲載漏れは静かに増える。
 *
 * **このガードは「載せろ」とは言わない。** T224 の方針は 2026-08-28 に ③ で決まった
 * ── **docs ページは「重要なバリエーションを選んで見せる場所」**であって全ストーリーの
 * 一覧ではない（ストーリーはサイドバーに必ず出る）。`docs/rules/mdx.md` の「ストーリー掲載時の
 * 注意点」に明文化してある。**欠陥なのは「そのページに例が 1 つも無い」場合だけ**で、
 * 起票時に 4 件（`Blockquote` / `CreditCardInput` / `CalendarHeatmap` /
 * `CounterTextarea`）あったものは同日に直した（188 → 177）。
 * 残りは**意図せず増えたときに気付ける**ように凍結しているだけ。
 *
...
console.log(
  `\n✓ 掲載漏れは増えていません（${missing.length} 件で凍結 — docs ページは選んで見せる場所なので、これは方針どおりの状態）。`,
);
