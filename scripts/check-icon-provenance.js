 ...
 * （残り 6 個はどれとも一致しなかった）。**ここに無いセットと一致した場合は
 * 素通りする** ── 「一致なし」は「自前」の証明ではない。
 *
 * その限界を埋めるため、**残った 6 個は別途 `@iconify/json`（236 セット・
 * 328,420 個）とも突き合わせた**（2026-08-05・一致なし）。Iconify は 800MB
 * 級なので常設の照合先にはできない。ここに置く 5 セットは「日常的に混入
 * しうる範囲」で、それより広い照合は疑いが出たときに手で回す。
 */
