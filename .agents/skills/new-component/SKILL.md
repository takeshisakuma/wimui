...
1. `SKILLS.md` の「コンポーネント新規作成の手順」を読む。**ファイルは手で作らず `npm run scaffold -- <Name> <category>` で作る**（`forwardRef` + `asChild`・`@layer component`・MDX 雛形・`react-i18next` のモック・`wim-<kebab名>` フックが入る。手で作ると `check:hooks` / `audit-mdx` などで落ちる）。
2. 配置・バレル・peer 依存の置き場は `AGENTS.md`「ファイル・エクスポート」と、`SKILLS.md` の同じ手順の 1 番。**peer を eager に import する部品は `*-core.ts` やルートに載せない。**
